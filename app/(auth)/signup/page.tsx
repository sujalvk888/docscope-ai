...
export default function SignupPage() {
  ...
  return (
    <div className="flex min-h-screen w-full font-sans bg-[#EBE3C3]">
      ...
      {/* LEFT PANEL: Branding & Aesthetics (Identical to Login to prevent re-rendering visual jumps) */}
      ...
      <div className="relative flex w-full lg:w-1/2 flex-col items-center justify-center p-6 sm:p-12">
        ...
        <div className="w-full max-w-[400px] animate-in fade-in zoom-in-[0.98] slide-in-from-bottom-8 duration-700 ease-out">
          <div className="space-y-2 mb-8 sm:mb-10 text-center lg:text-left">
            <h2 className="text-2xl sm:text-3xl font-extrabold text-[#1A1515] tracking-tight">Create an account</h2>
            <p className="text-[#73615A] text-sm sm:text-base font-medium">Start transforming your documents today.</p>
          </div>
          ...
          <form onSubmit={handleSignup} className="space-y-5 sm:space-y-6">
            ...
            <Button 
              className="w-full text-base sm:text-lg py-6 sm:py-7 rounded-xl mt-2 sm:mt-4 bg-[#DB6E4C] hover:bg-[#C25A3A] text-[#F5F2E8] shadow-xl shadow-[#DB6E4C]/20 font-bold transition-transform active:scale-[0.98]" 
              type="submit" 
              disabled={isLoading}
            >
              {isLoading ? <Loader2 className="mr-2 sm:mr-3 h-4 w-4 sm:h-5 sm:w-5 animate-spin text-[#F5F2E8]" /> : null}
          ...
          <div className="mt-8 sm:mt-10 text-center border-t border-[#D3C9AA]/60 pt-6 sm:pt-8">
            <p className="text-[#73615A] text-sm sm:text-base font-medium">
              ...
              <Link href="/login" className="text-[#1A1515] hover:text-[#DB6E4C] font-bold hover:underline underline-offset-4 transition-colors">
                Sign in
              </Link>
            </p>
          </div>
